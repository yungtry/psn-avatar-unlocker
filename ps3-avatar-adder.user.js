...
(function () {
    ...
    // Log load event in console (visible in F12)
    ...
    const CLIENT_NAME = '@sie-ppr-web-checkout/app';
    const CLIENT_VERSION = '2.176.0';
    const OPERATION_NAME = 'addToCart';
    const HASH_KEY = 'psa_addToCart_hash';
    const DEFAULT_HASH = '';

    // =========================================================================
    // HELPERS (defined at the top for frame and main window context)
    // =========================================================================
    ...
    // Generate random UUID for headers
    function uuid() {
    ...
    // =========================================================================
    // FETCH & XHR INTERCEPTION (document-start)
    // =========================================================================
    ...
    if (pageWindow.fetch) {
        ...
        pageWindow.fetch = function (...args) {
            try {
                ...
                if (url) {
                    interceptUrl(url);
                    if (init?.headers) interceptHeaders(init.headers);
                    if (init?.body) interceptBody(init.body);
                }
            } catch (_) { }
    ...
    if (pageWindow.XMLHttpRequest) {
        ...
        pageWindow.XMLHttpRequest.prototype.setRequestHeader = function (name, value, ...rest) {
            try {
                ...
                if (lowerName === 'apollographql-client-name') {
                    GM_setValue('psa_client_name', value);
                } else if (lowerName === 'apollographql-client-version') {
                    GM_setValue('psa_client_version', value);
                }
            } catch (_) { }
        ...
        pageWindow.XMLHttpRequest.prototype.send = function (body) {
            if (this._psaUrl && body) interceptBody(body);
            return originalSend.call(this, body);
        };
    }

    function interceptHeaders(headers) {
        if (!headers) return;
        try {
            let clientName = null;
            ...
            if (clientName && typeof clientName === 'string') {
                GM_setValue('psa_client_name', clientName.trim());
            }
            if (clientVersion && typeof clientVersion === 'string') {
                GM_setValue('psa_client_version', clientVersion.trim());
            }
        } catch (_) { }
    }
    ...
    function interceptUrl(url) {
        try {
            ...
            if (opMatch && hashMatch) {
                sendInterceptionNotice(opMatch[1], hashMatch[1]);
            }
        } catch (_) { }
    }

    function interceptBody(raw) {
        try {
    ...
    function sendInterceptionNotice(op, hash) {
        if (op === OPERATION_NAME) {
            if (/^[a-f0-9]{64}$/.test(hash)) {
                GM_setValue(HASH_KEY, hash);
            }
        }
    ...
    // =========================================================================
    // STOP IF IN IFRAME (UI only runs in top window)
    // =========================================================================
    if (window.self !== window.top) {
        return;
    }
    ...
    function addToCartGQL(sku, hash, country, language) {
        return new Promise((resolve) => {
            const locale = `${language.split('-')[0]}-${country}`;
            const clientName = GM_getValue('psa_client_name', CLIENT_NAME);
            const clientVersion = GM_getValue('psa_client_version', CLIENT_VERSION);
    ...
    function createUI() {
        ...
        const currentHashVal = GM_getValue(HASH_KEY, DEFAULT_HASH);
        ...
        document.getElementById('psa-add-btn').addEventListener('click', handleAdd);
        ...
        document.getElementById('psa-save-hash').addEventListener('click', () => {
            ...
            if (/^[a-f0-9]{64}$/.test(val)) {
                GM_setValue(HASH_KEY, val);
                updateUIHash(val);
                logMessage('ok', 'Hash saved manually.');
            } else {
                logMessage('err', 'Invalid hex hash.');
            }
        });
        document.getElementById('psa-delete-hash').addEventListener('click', () => {
            GM_setValue(HASH_KEY, '');
            updateUIHash('');
            logMessage('warn', 'Hash removed from memory.');
        });
        document.getElementById('psa-add-raw-btn').addEventListener('click', handleAddRaw);
        document.getElementById('psa-raw-sku-input').addEventListener('keydown', (e) => {
            if (e.key === 'Enter') handleAddRaw();
        });

        // Listen for postMessage from iframe
        window.addEventListener('message', (event) => {
            if (event.data && event.data.type === 'PSA_OP_INTERCEPTED') {
                handleInterceptedOp(event.data.op, event.data.hash);
            }
        });

        // Periodically check GM_getValue (in case localStorage updated from iframe)
        setInterval(() => {
            const h = GM_getValue(HASH_KEY, DEFAULT_HASH);
        ...
        if (currentHashVal) {
            logMessage('ok', 'Active hash loaded.');
        } else {
            logMessage('info', 'No hash. Script is listening in the background.');
            logMessage('info', 'Add any product to cart to capture it.');
        }
    }

    function updateUIHash(hash) {
    ...
    function handleInterceptedOp(op, hash) {
        // Log all intercepted ops to show users that the interceptor is working
        logMessage('info', `[Intercepted] ${op}: ${hash.substring(0, 12)}...`);

        if (op === OPERATION_NAME) {
            logMessage('ok', '🎉 Found addToCart hash!');
            updateUIHash(hash);
        }
    }
    ...
    async function handleAdd() {
        ...
        const activeHash = GM_getValue(HASH_KEY, DEFAULT_HASH);

        if (!avatarId) {
            logMessage('err', 'Enter avatar ID!');
            return;
        }

        if (!activeHash) {
            logMessage('err', 'No active hash.');
            logMessage('warn', 'Add any product to cart first to capture the hash.');
            return;
        }
        ...
        logMessage('info', `ID: ${avatarId}`);
        logMessage('info', `Region: ${country} / ${language}`);

        const suffixes = ['-E001', '-E002', '-E003', ''];

        for (const sfx of suffixes) {
            const sku = avatarId + sfx;
            logMessage('info', `Attempt: ${sku}...`);

            const data = await addToCartGQL(sku, activeHash, country, language);

            if (data?.data?.addToCart) {
                logMessage('ok', `✓ Added successfully: ${sku}`);
                logMessage('ok', '🎉 Success! Item is in your cart.');
                btn.disabled = false;
            ...
            if (data?.errors) {
                ...
                if (code === 'PERSISTED_QUERY_NOT_FOUND') {
                    logMessage('err', 'Outdated hash detected.');
                    logMessage('warn', 'Cleared invalid hash. Add a product to cart to fetch a new one.');
                    GM_setValue(HASH_KEY, '');
                    updateUIHash('');
                    btn.disabled = false;
                    btn.classList.remove('psa-loading');
                ...
                if (!msg.includes('SKU not found')) {
                    if (msg.includes('storefront') || msg.includes('store-front') || msg.includes('Store Front')) {
                        logMessage('ok', '⚠️ Received storefront error. Legacy PS3/PS4 avatars are often still successfully added to cart despite this! Check your cart on the official site.');
                    }
                    btn.disabled = false;
                    btn.classList.remove('psa-loading');
                    return;
        ...
        logMessage('err', 'All attempts failed. Check ID and region.');
        btn.disabled = false;
        btn.classList.remove('psa-loading');
    }
    ...
    async function handleAddRaw() {
        ...
        logMessage('info', `[Raw] SKU: ${rawSku}`);
        logMessage('info', `Region: ${country} / ${language}`);
        logMessage('info', `Attempting raw addition...`);

        const data = await addToCartGQL(rawSku, activeHash, country, language);

        if (data?.data?.addToCart) {
            logMessage('ok', `✓ Added successfully: ${rawSku}`);
            logMessage('ok', '🎉 Success! Item is in your cart.');
        } else if (data?.errors) {
            const err = data.errors[0];
            const msg = err?.message || 'Error';
            logMessage('err', `Failed: ${msg}`);
            if (msg.includes('storefront') || msg.includes('store-front') || msg.includes('Store Front')) {
                logMessage('ok', '⚠️ Storefront error: Legacy PS3/PS4 avatars are often still successfully added to cart despite this! Check your cart on the official site.');
            }
        } else {
            logMessage('err', 'Unknown response from server.');
        }
